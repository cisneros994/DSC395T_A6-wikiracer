            ...
            if link not in visited:
                if algorithm == "dfs":
                    ...
                    if link in goal_links and link not in common_links:
                        priority -= 1000
                    # Penalize frequently seen links
                    if link in common_links:
                        priority -= 500
                    # Prioritize substrings of goal in link or vise-versa
                    if goal[6:] in link[6:] or link[6:] in goal[6:] and (len(link[6:]) >= 3 and len(goal[6:]) >= 3):
                        priority -= 300
                    # Penalize longer links
                    priority += len(link[6:])*2

                    # cumulative cost + shorter links preferred + priorities
                    new_cost = current_cost + priority
                    heapq.heappush(container, (new_cost, link, path + [link]))
